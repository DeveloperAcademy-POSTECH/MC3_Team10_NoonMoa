...
struct AptView: View {
    ...
    @State private var isSettingViewOpen: Bool = false
            ...
            ZStack {
                ...
                if isSettingViewOpen {
                    ZStack {
                        ...
                        VStack {
                            SettingView(isSettingViewOpen: $isSettingViewOpen)
                                .frame(height: UIScreen.main.bounds.width * 1)
                                .padding()
                                .environmentObject(characterViewModel)
                            Spacer()
                ...
                GeometryReader { proxy in
                    HStack (spacing: 16) {
                        ...
                        NavigationLink(destination: SettingView(isSettingViewOpen: $isSettingViewOpen)) {
                            Button { // 설정 버튼
                                isSettingViewOpen.toggle()
                            } label: {
                                if isSettingViewOpen {
                                    Image.assets.buttons.settingSelected
                                        .resizable()
